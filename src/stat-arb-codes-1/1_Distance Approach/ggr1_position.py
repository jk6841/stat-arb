...
comnam_map = (
    stock_id[["namedt", "permno", "comnam"]].drop_duplicates().groupby(["permno"])["comnam"].last()
)
...
for d in ds:
    ...
    data = sp500_prices[permno_list].loc[
        d - relativedelta(years=1) : d + relativedelta(months=6) - relativedelta(days=1)
    ]
    ...
    # nomalized prices
    norm_formation_prices = (formation_prices - formation_prices.min()) / (
        formation_prices.max() - formation_prices.min()
    )
    norm_test_prices = (test_prices - formation_prices.min()) / (
        formation_prices.max() - formation_prices.min()
    )
    ...
    # ssd 계산
    ssd = pd.DataFrame(
        index=np.arange(n_cols * (n_cols - 1) / 2, dtype=int), columns=["no1", "no2", "ssd", "std"]
    )
    c = 0
    ...
    for i in range(len(ssd_n)):
        ...
        norm_price1, norm_price2 = (
            norm_test_prices[no1].loc[test_prices12.index],
            norm_test_prices[no2].loc[test_prices12.index],
        )
        ...
        # 당일 매매 주문 데이터 생성
        entry = (position[no1] != 0.0) & (
            (position[no1].shift(1) == 0.0) | (position[no1].shift(1).isnull())
        )
        exit = (
            (position[no1] == 0.0)
            & (position[no1].shift(1) != 0.0)
            & (position[no1].shift(1).notnull())
        )
        order = position.diff()
...
orders.to_pickle("./orders_top30.pkl")
orders_delay.to_pickle("./orders_delay_top30.pkl")
